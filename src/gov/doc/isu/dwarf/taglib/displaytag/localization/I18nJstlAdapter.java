 ...
 */
package gov.doc.isu.dwarf.taglib.displaytag.localization;
...
/**
 * JSTL implementation of a resource provider and locale resolver. It will make the <code>titleKey</code> attribute of column tag works the same as fmt:message's <code>key property</code>. This tag must be the descendant of a <code>fmt:bundle</code> tag in order to use the titleKey. This is just a shortcut, which makes
 * 
 * <pre>
 ...
 * </pre>
 * 
 * If you don't define either <code>titleKey</code> or <code>titleKey</code> property on your column, first the tag will attempt to look up the <code>property</code> property in your ResourceBundle. Failing that, it will fall back to the parent class's behavior of just using the property name.
 * 
...
public class I18nJstlAdapter implements I18nResourceProvider, LocaleResolver {
     ...
     * logger.
     */
    private static Logger log = Logger.getLogger(I18nJstlAdapter.class);
