...
import gov.doc.isu.dwarf.taglib.displaytag.render.TableWriterTemplate;
import gov.doc.isu.dwarf.taglib.displaytag.util.LookupUtil;
import gov.doc.isu.dwarf.taglib.displaytag.util.TagConstants;

/**
 * This is a hte main abstract class for decorting table values
 * 
 * @author epesh
...
public abstract class TableDecorator extends Decorator {
    ...
    /**
     * Return the index in the full list (view index + offset). Note that the index returned if from the <strong>sorted</strong> list, and not from the original one.
     * 
