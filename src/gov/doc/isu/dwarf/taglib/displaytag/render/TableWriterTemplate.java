...
public abstract class TableWriterTemplate {
     ...
     * logger.
     */
    private static Logger log = Logger.getLogger(TableWriterTemplate.class);
     ...
     * @throws JspException
     *         if any exception thrown while constructing the tablei, it is caught and rethrown as a JspException. Extension classes may throw all sorts of exceptions, depending on their respective formats and destinations.
     */
    public void writeTable(TableModel model, String id) throws JspException {
        try{
            ...
            writeExpandCollapseAllBanner(model);
            // open table
    ...
    protected abstract void writeExpandCollapseAllBanner(TableModel model) throws Exception;
    ...
    /**
     * Given a table model, writes the table body content, sorting and grouping it per its configuration, while delegating where and how it writes to subclass objects. (Background: This method refactors TableTagData.writeTableBody method. See above.)
     * 
     * @param model
    ...
    private void writeTableBody(TableModel model) throws Exception {
        ...
        while(nextRow != null || rowIterator.hasNext()){
            ...
            /******* Enhancement :: Search Row :: Start *******/
            /** display any subrows above the main row */
            for(int i = 0;i < currentRow.getSearchRows().size();i++){
                FilterSearchRow filterSearchRow = (FilterSearchRow) currentRow.getSearchRows().get(i);
                if((filterSearchRow.getFrequency() == FilterSearchRow.DISPLAY_ALWAYS) || ((filterSearchRow.getFrequency() == FilterSearchRow.DISPLAY_FIRST) && searchFirstrow) || ((filterSearchRow.getFrequency() == FilterSearchRow.DISPLAY_LAST) && !rowIterator.hasNext())){
                    writeSearchRowOpener(filterSearchRow);
                    writeSearchRowWithNoColumns(filterSearchRow.getRowContent().toString());
                    writeSearchRowCloser(filterSearchRow);
                }
            }
            /******* Enhancement :: Search Row :: End *******/
            ...
            /******* Enhancement :: Sub Row :: Start *******/
            /** display any subrows above the main row */
            for(int i = 0;i < currentRow.getSubRowsAbove().size();i++){
                SubRow subRow = (SubRow) currentRow.getSubRowsAbove().get(i);
                if((subRow.getFrequency() == SubRow.DISPLAY_ALWAYS) || ((subRow.getFrequency() == SubRow.DISPLAY_FIRST) && firstrow) || ((subRow.getFrequency() == SubRow.DISPLAY_LAST) && !rowIterator.hasNext())){
                    writeSubRowOpener(subRow);
                    writeSubRowWithNoColumns(subRow.getRowContent().toString());
                    writeSubRowCloser(subRow);
                }
            }
            /******* Enhancement :: Sub Row :: End *******/
            // open row
            ...
            for(Iterator iterator = structsForRow.iterator();iterator.hasNext();){
                ...
                String decoratedValue = struct.decoratedValue;
                if(model.getToggle() != null && (model.getToggle().equals("collapsed") || model.getToggle().equals("expanded")) && !iterator.hasNext() && currentRow.getSubRowsBelow() != null && currentRow.getSubRowsBelow().size() != 0){
                    StringBuffer sb = new StringBuffer("<div style=\"float:right;\"><span style=\"width:70px\" class=\"enableWithRestrict pointerCursor\" ");
                    sb.append(" onclick=\"toggleHandler('").append(model.getId()).append(currentRow.getRowNumber()).append("')\" ><img class=\"expCol").append(model.getId()).append("\" id=\"img").append(model.getId()).append(currentRow.getRowNumber()).append("\"  style=\"float:right;hspace:0;vspace:0;\" src=\"").append(model.getContextPath()).append("/jsp/images/");
                    if(model.getToggle().equals("collapsed")){
                        sb.append("colapse");
                    }else{
                        sb.append("expand");
                    }
                    sb.append(".svg\"></span></div>");
                    decoratedValue += sb.toString();
                }
                writeColumnValue(decoratedValue, struct.column);
            ...
            /******* Enhancement :: Sub Row :: Start *******/
            /** display any subrows after the main row */
            for(int i = 0;i < currentRow.getSubRowsBelow().size();i++){
                SubRow subRow = (SubRow) currentRow.getSubRowsBelow().get(i);
                subRow.setId(model.getId() + currentRow.getRowNumber());
                if((subRow.getFrequency() == SubRow.DISPLAY_ALWAYS) || ((subRow.getFrequency() == SubRow.DISPLAY_FIRST) && firstrow) || ((subRow.getFrequency() == SubRow.DISPLAY_LAST) && !rowIterator.hasNext())){
                    writeSubRowOpener(subRow);
                    writeSubRowWithNoColumns(subRow.getRowContent().toString());
                    writeSubRowCloser(subRow);
            ...
            /******* Enhancement :: Search Row :: Start *******/
            /** This is added to for searchRow Display Tag Enhancement **/
            searchFirstrow = false;
            /******* Enhancement :: Search Row :: End *******/

            /******* Enhancement :: Sub Row :: Start *******/
            /** This is added to for subRow Display Tag Enhancement **/
            firstrow = false;
            /******* Enhancement :: Sub Row :: End *******/
        }
        ...
        if(model.getRowListPage().size() == 0){
            writeEmptyListRowMessage(MessageFormat.format(model.getProperties().getEmptyListRowMessage(), new Object[]{new Integer(model.getNumberOfColumns())}));
        }
    }
    ...
    /******* Enhancement :: Search Row :: Start *******/
    /**
     * Called by writeTableBody to write the start of the search row structure.
     * 
     * @param row
     *        The table FilterSearchRow for which the content is written.
     * @throws Exception
     *         if it encounters an error while writing.
     */
    protected abstract void writeSearchRowOpener(FilterSearchRow row);

    /**
     * Called by writeTableBody to write a search row that has no columns.
     * 
     * @param searchRowContent
     *        The search row value.
     * @throws Exception
     *         if it encounters an error while writing.
     */
    protected abstract void writeSearchRowWithNoColumns(String searchRowContent);

    /**
     * Called by writeTableBody to write the end of the search row structure.
     * 
     * @param row
     *        The table FilterSearchRow for which the content is written.
     * @throws Exception
     *         if it encounters an error while writing.
     */
    protected abstract void writeSearchRowCloser(FilterSearchRow row);

    /******* Enhancement :: Search Row :: End *******/

    /******* Enhancement :: Sub Row :: Start *******/
    /**
     * Called by writeTableBody to write the start of the sub row structure.
     * 
     * @param row
     *        The table SubRow for which the content is written.
     * @throws Exception
     *         if it encounters an error while writing.
     */
    protected abstract void writeSubRowOpener(SubRow row);

    /**
     * Called by writeTableBody to write a sub row that has no columns.
     * 
     * @param subRowContent
     *        The sub row value.
     * @throws Exception
     *         if it encounters an error while writing.
     */
    protected abstract void writeSubRowWithNoColumns(String subRowContent);

    /**
     * Called by writeTableBody to write the end of the sub row structure.
     * 
     * @param row
     *        The table SubRow for which the content is written.
     * @throws Exception
     *         if it encounters an error while writing.
     */
    protected abstract void writeSubRowCloser(SubRow row);
