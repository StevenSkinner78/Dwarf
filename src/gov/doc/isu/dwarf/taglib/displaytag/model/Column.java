...
import gov.doc.isu.dwarf.taglib.displaytag.util.HtmlTagUtil;
...
public class Column {
    ...
    public Object getValue(boolean decorated) throws ObjectLookupException, DecoratorException {
        ...
        if(this.cell.getStaticValue() != null){
            ...
            // decorator
            if(decorated && this.row.getParentTable().getTableDecorator() != null && this.row.getParentTable().getTableDecorator().hasGetterFor(this.header.getBeanPropertyName())){

                object = LookupUtil.getBeanProperty(this.row.getParentTable().getTableDecorator(), this.header.getBeanPropertyName());
            }else{
        ...
        if(decorated){
            for(int j = 0;j < decorators.length;j++){
                object = decorators[j].decorate(object, row.getParentTable().getPageContext(), row.getParentTable().getMedia());
            }
        }
    ...
    private Href getColumnHref(String columnContent) throws ObjectLookupException {
        ...
        if(this.header.getParamName() != null){
            ...
            if(paramValue != null){
                try{
                    colHref.addParameter(this.header.getParamName(), URLEncoder.encode(paramValue.toString(), StringUtils.defaultString(this.row.getParentTable().getEncoding(), "UTF8"))); //$NON-NLS-1$
