...
import gov.doc.isu.dwarf.taglib.displaytag.exception.ExportException;
...
import gov.doc.isu.dwarf.taglib.displaytag.export.BinaryExportView;
import gov.doc.isu.dwarf.taglib.displaytag.export.ExportView;
import gov.doc.isu.dwarf.taglib.displaytag.export.ExportViewFactory;
import gov.doc.isu.dwarf.taglib.displaytag.export.TextExportView;
import gov.doc.isu.dwarf.taglib.displaytag.model.Cell;
...
import gov.doc.isu.dwarf.taglib.displaytag.pagination.PaginatedListSmartListHelper;
import gov.doc.isu.dwarf.taglib.displaytag.pagination.PaginationHelper;
...
import gov.doc.isu.dwarf.taglib.displaytag.render.HtmlTableWriter;
import gov.doc.isu.dwarf.taglib.displaytag.util.CollectionUtil;
import gov.doc.isu.dwarf.taglib.displaytag.util.DependencyChecker;
import gov.doc.isu.dwarf.taglib.displaytag.util.Href;
import gov.doc.isu.dwarf.taglib.displaytag.util.ParamEncoder;
import gov.doc.isu.dwarf.taglib.displaytag.util.RequestHelper;
...
public class TableTag extends HtmlTableTag {
    ...
    "gov.doc.isu.dwarf.taglib.displaytag.filter.ResponseOverrideFilter.CONTENT_OVERRIDE_BODY"; //$NON-NLS-1$
     ...
     *        - The value of exportColOffset to be set.
     */
     ...
     */
    public void setOffenderPage(boolean offenderPage) {
     ...
     * 
     * @param value
     *        name of the object to use for iteration (can contain expression). It also supports direct setting of a list, for jsp 2.0 containers where users can set up a data source here using EL expressions.
     */
    ...
    public int doStartTag() throws JspException {
        ...
        this.tableModel.setTableName(this.tableName);
        this.tableModel.setNoRecords(this.noRecords);
        initParameters();
    ...
    /**
     * Get the given parameter from the request or, if not avaible, look for into into the session if keepstatus is set. Also takes care of storing an existing paramter into session.
     * 
     * @param request
     *        servlet request
     * @param requestHelper
    ...
    private void initParameters() throws JspTagException, FactoryInstantiationException {
        ...
        if(!this.tableModel.isLocalSort()){
            ...
            Integer sortColumnParameter = getFromRequestOrSession(request, requestHelper, TableTagParameters.PARAMETER_SORT);
        ...
        if(this.paginatedList == null){
            SortOrderEnum paramOrder = SortOrderEnum.fromCode(getFromRequestOrSession(request, requestHelper, TableTagParameters.PARAMETER_ORDER));
        ...
        Integer exportTypeParameter = requestHelper.getIntParameter(encodeParameter(TableTagParameters.PARAMETER_EXPORTTYPE));

        this.currentMediaType = (MediaTypeEnum) ObjectUtils.defaultIfNull(MediaTypeEnum.fromCode(exportTypeParameter), MediaTypeEnum.HTML);
        ...
        if(this.getToggle() != null){
            this.tableModel.setToggle(this.getToggle());
        }
        /* If toggle all is set then it is for both row level toggle and for Expand All and collapse All** */
        if(this.getToggleAll() != null){
            this.tableModel.setToggleAll(this.getToggleAll());
            this.tableModel.setToggle(this.getToggleAll());
        }
        this.tableModel.setContextPath(((HttpServletRequest) pageContext.getRequest()).getContextPath());
    }

    /**
     * Is the current row included in the "to-be-evaluated" range? Called by nested ColumnTags. If <code>false</code> column body is skipped.
     * 
     * @return <code>true</code> if the current row must be evaluated because is included in output or because is included in sorting.
     */
    ...
    /**
     * Create a complete string for compatibility with previous version before expression evaluation. This approach is optimized for new expressions, not for previous property/scope parameters.
     * 
    ...
    protected void initHref(RequestHelper requestHelper) {
        ...
        if(this.excludedParams != null){
            ...
            if(splittedExcludedParams.length == 1 && "*".equals(splittedExcludedParams[0])){
                // Remove parameters from baseHref which are not added by TableTag
                removeParameters();
            }else{
        ...
        if(this.requestUri != null){
            ...
            if(!this.dontAppendContext){
                ...
                if(!StringUtils.isEmpty(contextPath) && requestUri != null && requestUri.startsWith("/") && !requestUri.startsWith(contextPath)){
    ...
    public int doEndTag() throws JspException {
        ...
        TableDecorator tableDecorator = this.properties.getDecoratorFactoryInstance().loadTableDecorator(this.pageContext, getConfiguredDecoratorName());
        ...
        this.tableModel.setExportColOffset(MediaTypeEnum.HTML.equals(this.currentMediaType) ? 0 : this.exportColOffset);
        ...
        if(MediaTypeEnum.HTML.equals(this.currentMediaType) && (previousMediaType == null || MediaTypeEnum.HTML.equals(previousMediaType))){
            ...
            if(this.isOffenderPage()){
                HttpServletRequest request = (HttpServletRequest) this.pageContext.getRequest();
                HttpSession session = request.getSession();
                this.tableModel.setOffenderInfo((CommonModel) session.getAttribute("offenderInfo"));
    ...
    /**
     * Returns the name of the table decorator that should be applied to this table, which is either the decorator configured in the property "decorator", or if none is configured in said property, a decorator configured with the "decorator.media.[media type]" property, or null if none is configured.
     * 
     * @return Name of the table decorator that should be applied to this table.
     */
    private String getConfiguredDecoratorName() {
        String tableDecoratorName = (this.decoratorName == null) ? this.properties.getMediaTypeDecoratorName(this.currentMediaType) : this.decoratorName;
        tableDecoratorName = (tableDecoratorName == null) ? this.properties.getExportDecoratorName(this.currentMediaType) : tableDecoratorName;
    ...
    /**
     * If no columns are provided, automatically add them from bean properties. Get the first object in the list and get all the properties (except the "class" property which is automatically skipped). Of course this isn't possible for empty lists.
     */
    ...
    protected void setupViewableData() {
        ...
        if(this.paginatedList == null && this.pagesize > 0){
            this.listHelper = new SmartListHelper(fullList, (this.partialList) ? ((Integer) size).intValue() : fullList.size(), this.pagesize, this.properties, this.partialList);
    ...
    protected void writeHTMLData() throws JspException {
        ...
        new HtmlTableWriter(this.tableModel, this.properties, this.baseHref, this.export, out, getCaptionTag(), this.paginatedList, this.listHelper, this.pagesize, getAttributeMap(), this.uid).writeTable(this.tableModel, this.getUid());
    ...
    /**
     * Adds a sub row to a row. This method is usually called by a contained FilterSearchRowTag.
     * 
     * @param filterSearchRow
     *        to add to the current row.
     */
    public void addSearchRow(FilterSearchRow filterSearchRow) {
        if(this.currentRow != null){
            this.currentRow.addSearchRow(filterSearchRow);
        }
    }
    ...
    /**
     * Adds a sub row to a row. This method is usually called by a contained SubRowTag.
     * 
     * @param subRow
     *        to add to the current row.
     */
    public void addSubRow(SubRow subRow) {
        if(this.currentRow != null){
            this.currentRow.addSubRow(subRow);
        }
    }

    /******* Enhancement :: Sub Row :: End *******/

    public List<ColumnModel> getTheCurrentRowColumns() {
        CommonModel model = (CommonModel) this.currentRow.getObject();
        return model.getColumnData();
    }
}
